...
mv -f "/tmp/$aiinaalpkg/sdxl_styles/"* "$AIROCmALdir/shared1/sdxl_styles/"
cd $AIROCmALdir/$aiinaalpkg
git checkout .
rm -r /tmp/$aiinaalpkg
...
echo "python $AIROCmALdir/$aiinaalpkg/main.py --port 7865 \"\$@\"" >> $AIROCmALdir/$aiinaalpkg/$aiinaalpkg-Start.sh
echo "sleep 60"
...
echo "Installation complete. Start with command: AIROCmAL-$aiinaalpkg with any ComfyUI arguments afterward"
